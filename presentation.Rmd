...
rast <- sf::st_intersection(x = sf::st_as_sf(rast), y = postcode6Shape)[, c(1:40,
...
```{r voro}
voronoi <- sf::st_voronoi(x = pollstations %>% st_combine(), envelope = NULL  # this is in case we want to specify the Voronoi boundaries
)
...
```{r, cache=TRUE}
rast <- sf::st_intersection(x = sf::st_as_sf(rast), y = voronoi)
```
...
```{r}
map_dfr(slopes, get_scores_seg_pol) %>% filter(-0.2>=cor)
```
...
leaflet(voronoi2_sf) %>%
  ...
  addPolygons(
    ...
    opacity = 0.7,
    options = pathOptions(pane = "right"), group = "moran") %>% 
   addPolygons(
    label = ~d_5,
    color = ~palette2(d_5),
    opacity = 0.7,
  ...
  addLayersControl(overlayGroups = c("Pvar","pollstation", "d")) %>%
